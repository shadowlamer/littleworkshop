...
void draw_sprite(t_sprite_id sprite_id, uint8_t x, uint8_t y) {
  ...
  // --- Отрисовка спрайта ---
  for (uint8_t line = 0; line < spr->height * 8; line++) {
    p_screen = (uint8_t*)(screen_line_addrs[line + y] + screen_x_byte);
    for (uint8_t col = 0; col < spr->width; col++) {
      if (screen_x_bit == 0) {
        // Без сдвига
...
void draw_simple_sprite(t_sprite_id sprite_id, uint8_t x, uint8_t y) {
  const t_sprite *spr = sprites + sprite_id;
  ...
  for (uint8_t line = 0; line < spr->height * 8; line++) {
    p_screen = (uint8_t*)(screen_line_addrs[line + y] + x);
    memcpy(p_screen, p_sprite_data, spr->width);
    p_sprite_data += spr->width;
  }  
}
